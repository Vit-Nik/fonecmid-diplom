...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый() Тогда
		Исполнитель = ЭтотОбъект.Специалист;
		ДатаРабот = ЭтотОбъект.ДатаПроведенияРабот;
		ВремяНачалаРабот = ЭтотОбъект.ВремяНачалаРаботПлан;
		
		ТекстСообщения = СтрШаблон("Создан новый документ обслуживание клиента. Исполнитель %1, дата исполнения %2, время начала работ %3",
		Исполнитель, ДатаРабот, ВремяНачалаРабот);
